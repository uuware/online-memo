...
//Inner Error Number
define( 'DB_ERR_NOTAVAILABLE', 1 );
define( 'DB_ERR_NOTCONNECT', 2 );
define( 'DB_ERR_NOTSELECTDB', 3 );
define( 'DB_ERR_NOTWHERE', 9 );
...
class db_base
{
	...
	*/
	function &getInstance( $options	= array() )
	{
		...
		$newinst = isset($options['new']) && $options['new'];

		if (empty($instances[$signature]) || $newinst)
		{
			...
			$path	= dirname(__FILE__).DS.'db_'.$driver.'.php';

			if (file_exists($path)) {
				require_once($path);
			} else {
				//die( 'No db class:'.$path );
				$error = 'Unable to load Database Driver:'.$driver;
				return $error;
				//StBase::setErrorHandling(E_ERROR, 'die'); //force error type to die
				//$error = StBase::raiseError( 500, JTEXT::_('Unable to load Database Driver:') .$driver);
				//return $error;
			}
			...
			$instance->_trace = defined( '_ST_TRACE' ) && _ST_TRACE;

			if (version_compare(PHP_VERSION, '5') == -1) {
				$instance->__construct($options);
			}
			if ( $error = $instance->getErrorMsg() )
			{
				//StBase::setErrorHandling(E_ERROR, 'ignore'); //force error type to die
				//$error = StBase::raiseError( 500, JTEXT::_('Unable to connect to the database:') .$error);
				//return $error;
				//die( 'No db class:'.$adapter );
				return $error;
			}
			if(!$newinst) {
	...
	function trace( $level ) {
		$oldtrace = $this->_trace;
		$this->_trace = intval( $level );
		return $oldtrace;
	}
	...
	*/
	...
	function insertObject( $table, &$object, $keyName = NULL )
	{
		...
		$object2 = &$object;
		if(is_object($object)) {
			$object2 = get_object_vars( $object );
		}
		foreach ($object2 as $k => $v) {
			if (is_array($v) || is_object($v) || $v === NULL || $k[0] == '_') { // internal or NA field
				continue;
			}
			$fields[] = $this->nameQuote( $k );
			$values[] = $this->isQuoted( $k ) ? $this->Quote( $v ) : (int) $v;
		}
		$fmtsql = 'INSERT INTO '.$this->nameQuote($table).' ( '.implode( ",", $fields ).' ) VALUES ( '.implode( ",", $values ).' ) ';
	 ...
	 */
	function updateObject( $table, &$object, $keyNameArr, $updateNulls=true )
	{
		settype($keyNameArr, 'array'); //force to array
		$tmp = array();
		$where = '';
		$object2 = &$object;
		if(is_object($object)) {
		...
		foreach ($object2 as $k => $v)
		{
			...
			if(substr($k, 0, 1) == '@') {
				//special for update like: version = version + 1
				$tmp[] = substr($k, 1).'='.$v;
				continue;
			}
			if( in_array($k, $keyNameArr) ) { // PK not to be updated
				if($where != '') {
					$where.= ' AND ';
				}
		...
		if($where == '') {
			$this->_errorNum = DB_ERR_NOTWHERE;
			$this->_errorMsg = 'Can not do update without WHERE filter.';
			return false;
		}
		$fmtsql = 'UPDATE '.$this->nameQuote($table).' SET '.implode( ",", $tmp ).' WHERE '.$where;
		$this->setQuery( $fmtsql );
		return $this->query();
	}
	 ...
	 * @access public
	 * @param [type] $table
	 * @param [type] &$whereFieldsValue
	 */
	function deleteObject( $table, $whereFieldsValue )
	{
		$where = '';
		foreach ($whereFieldsValue as $k => $v)
		{
			if($where != '') {
				$where.= ' AND ';
			}
		...
		if($where == '') {
			$this->_errorNum = DB_ERR_NOTWHERE;
			$this->_errorMsg = 'Can not do datele without WHERE filter.';
			return false;
		}
		$fmtsql = 'DELETE FROM '.$this->nameQuote($table).' WHERE '.$where;
		$this->setQuery( $fmtsql );
		return $this->query();
	}
	 ...
	 * @param [type] $whereFieldsValue
	 */
	function selectResult( $table, $selectFields, $whereFieldsValue )
	{
		return $this->selectObjectSub($table, $selectFields, $whereFieldsValue, 1);
	}
	 ...
	 * @param [type] $whereFieldsValue
	 */
	function selectObject( $table, $selectFields, $whereFieldsValue )
	{
		return $this->selectObjectSub($table, $selectFields, $whereFieldsValue, 2);
	}
	 ...
	 * @param [type] $whereFieldsValue
	 */
	function selectObjectList( $table, $selectFields, $whereFieldsValue )
	{
		return $this->selectObjectSub($table, $selectFields, $whereFieldsValue, 3);
	}
	 ...
	 */
	function selectObjectSub( $table, $selectFields, $whereFieldsValue, $resultType )
	{
		$select = '';
		$where = '';
		...
		if($where == '') {
			$this->_errorNum = DB_ERR_NOTWHERE;
			$this->_errorMsg = 'Can not do select without WHERE filter.';
			return false;
		}
		$fmtsql = 'SELECT '.$select.' FROM '.$this->nameQuote($table).' WHERE '.$where;
		$this->setQuery( $fmtsql );
		if($resultType == 1) {
	 ...
	 * @return array A list of all the tables in the database
	 */
	function getOwnerTableList($prefix='#__')
	{
		$pre = $this->getPrefix();
		$len = strlen($pre);
		...
		if($arr && count($arr) > 0) {
			foreach($arr as $table) {
				if(substr($table, 0, $len) == $pre) {
					$arr2[] = $prefix . strtoupper(substr($table, $len));
				}
			}
		}
		return $arr2;
	...
	*/
	...
	//must Quote($text) or nameQuote($text) first
	function QuoteSubstr( $text, $from, $len )
	{
		//Microsoft SQL Server
		//SELECT SUBSTRING('wabbit_duck_season', 7, 11)
		//MySQL
		//SELECT SUBSTRING('wabbit_duck_season', 7, 11); or SUBSTR
		//Oracle
		//SELECT SUBSTR('wabbit_duck_season', 7, 11) FROM dual;
		//PostgreSQL
		//SELECT SUBSTR('wabbit_duck_season', 7, 11);
		//Sqlite
		//SELECT SUBSTR('wabbit_duck_season', 7, 11);
		return 'SUBSTR('.$text.', '.$from.', '.$len.')';
	}
	//must Quote($text) or nameQuote($text) first
	function QuoteTrim( $text )
	{
		//Microsoft SQL Server
		//LTRIM(RTRIM('     sql_in_a_nutshell     '))
		//MySQL
		//SELECT TRIM(BOTH FROM '     sql_in_a_nutshell     ');
		//Oracle
		//SELECT TRIM('     sql_in_a_nutshell      ') FROM dual;
		//PostgreSQL
		//SELECT TRIM(BOTH FROM '     sql_in_a_nutshell     ');
		//Sqlite
